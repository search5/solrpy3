...
class TestAddingDocuments(SolrConnectionTestCase):
    ...
    def test_add_unicode(self):
        ...
        data = "\xc5\xbc\xc3\xb3\xc5\x82\xc4\x87"
    ...
    def test_add_many_unicode(self):
        ...
        chars = ("\xc4\x99\xc3\xb3\xc4\x85\xc5\x9b\xc5\x82"
                 "\xc4\x98\xc3\x93\xc4\x84\xc5\x9a\xc5\x81")
...
class TestSolrConnectionSearchHandler(SolrConnectionTestCase):
    ...
    def test_select_request(self):
        conn = self.new_connection()
        conn.select(b"id:foobar", score=False)
    ...
    def test_alternate_request(self):
        ...
        alternate(b"id:foobar", score=False)
...
class TestSolrAddingDocuments(SolrBased, RequestTracking, TestAddingDocuments):
    ...
    def test_add_noflush(self):
        ...
        self.assertEqual(
            self.selector(),
            "/update?commit=true&waitSearcher=false&waitFlush=false")
    ...
    def test_add_many_commit_noflush(self):
        ...
        self.assertEqual(
            sorted(self.selector().split("&")),
            sorted("/update?commit=true&waitSearcher=false&waitFlush=false".split("&")))
        # Can't verify the add since we said we weren't going to wait
        # for the flush.
        self.assertTrue(b"<add>" in self.postbody())
...
class SolrExceptionHttpStatusPickleTestCase(unittest.TestCase):
    ...
    def _test_unpickle(self, s):
        ...
        if type(s) == bytes:
            ...
            # print()
            loaded = self.module.loads(s.encode("utf-8"))
